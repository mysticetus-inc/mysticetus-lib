...
use crate::client::{BytesBody, HttpsClient};
...
impl ApplicationDefault {
    ...
    pub(crate) fn load_from_credentials_file_bytes_with_client(
        ...
        let proj_id = match (
            ...
            (_, Some(project_id)) => ProjectId::from(Box::from(project_id)),
            (Some(uri), None) => match find_project_id_in_impersonated_uri(uri) {
                Some(project_id) => ProjectId::from(Box::from(project_id)),
                None => {
                    return Err(Error::io(
    ...
    pub fn new() -> Option<TryLoadFuture<'static>> {
...
impl<'a> TryLoadFuture<'a> {
    ...
    pub(crate) fn into_static(self) -> TryLoadFuture<'static> {
        TryLoadFuture {
            https: self.https.take_into_static(),
            read_fut: self.read_fut,
        }
    }
}

impl Future for TryLoadFuture<'_> {
    type Output = crate::Result<Option<(ApplicationDefault, ProjectId)>>;

    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        let mut this = self.project();
        let bytes = match std::task::ready!(this.read_fut.poll(cx)) {
            Ok(bytes) => bytes,
            Err(err) if err.kind() == std::io::ErrorKind::NotFound => return Poll::Ready(Ok(None)),
            Err(err) => return Poll::Ready(Err(err.into())),
        };

        let (provider, proj_id) = ApplicationDefault::load_from_credentials_file_bytes_with_client(
            &mut this.https,
            &bytes,
        )?;

        Poll::Ready(Ok(Some((provider, proj_id))))
    }
}
