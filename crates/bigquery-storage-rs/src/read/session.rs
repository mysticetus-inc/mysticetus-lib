        ...
        while let Some(read_stream_result) = read_streams.next().await {
            ...
            handles.spawn(async move {
                while let Some(result) = stream.next().await {
                    // if the reciever bails, we should just bail asap
                    if stream_tx.send(result).is_err() {
                        break;
                    }
                }
        ...
        for stream in this.streams.iter_mut() {
            match stream.stream.poll_next_unpin(cx) {
                Poll::Ready(Some(item)) => {
                    if stream.completed {
                        tracing::error!("stream previously yielded None, but just yielded Some!");
                    }
...
impl<O> Stream for SpawnStreamAll<O> {
    ...
    fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        if !*this.rx_closed {
            ...
            tracing::debug!("stream rx closed");
        }

        loop {
            match ready!(this.handles.poll_join_next(cx)) {
                Some(Ok(Ok(_))) => {
                    tracing::debug!("joined task handle ({} remaining)", this.handles.len())
                }
