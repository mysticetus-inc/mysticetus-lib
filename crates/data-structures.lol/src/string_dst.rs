...
impl<const CAP: usize> StringDst for crate::stack_string::StackString<CAP> {
    type Ref<'a> = &'a str;
    ...
    fn handle_str(&mut self, string: &str) {
        if self.is_empty() {
            *self = crate::stack_string::StackString::from(string);
        } else {
            let _ = self.push_str(string);
        }
    }
    ...
    fn handle_string(&mut self, string: String) {
        ...
        if self.is_empty() {
            *self = crate::stack_string::StackString::from(string.as_str());
        } else {
            let _ = self.push_str(&string);
        }
    }
    ...
    #[inline]
    fn clear(&mut self) {
        crate::stack_string::StackString::clear(self);
    }
}
