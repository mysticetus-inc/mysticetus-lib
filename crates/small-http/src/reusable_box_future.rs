...
///
/// Code copied from `tokio-util` and modified to remove the [`Send`]
/// bound on the inner boxed [`Future`].
#[repr(transparent)]
...
impl<'a, T> ReusableBoxFuture<'a, T> {
        ...
        F: Future<Output = T> + 'a,
    {
            ...
            F: Future + 'a,
        {
            ...
            let boxed = std::mem::replace(&mut this.boxed, Box::pin(std::future::pending()));
    ...
    pub fn poll(&mut self, cx: &mut Context<'_>) -> Poll<T> {
        Pin::new(&mut self.boxed).poll(cx)
    }
}
...
impl<T> std::fmt::Debug for ReusableBoxFuture<'_, T> {
